...
invisible(mapply(
  ...
  x = head(rep(1:5 / 5, times = 3) - 1/10, 14),
  y = head(rep(3:1 / 3, each  = 5) - 1/6,  14),
  function (p, x, y) {
    gp <- gpar(fill = fill_pattern(p))
    grid.rect(x = x, y = y - .03, width = .16, height = .2, gp = gp)
...
invisible(mapply(
  p = c(
    ...
    "herringbone45", "hexagon_sm", "octagon_:sm", "rain90_xs", "saw_16mm:8mm", 
    "shingle_3lwd", "rshingle_dotted", "stripe35_lg", "wave45_sm", "empty_xl" ), 
  x = head(rep(1:5 / 5, times = 3) - 1/10, 14),
  y = head(rep(3:1 / 3, each  = 5) - 1/6,  14),
  function (p, x, y) {
    gp <- gpar(fill = fill_pattern(p))
